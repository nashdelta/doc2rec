...
# Necessary packages (should be commented when called within the parent script)
import pandas
...
def pmid2graph(table_in):
    ...
    # Import data
    data_in = pandas.read_csv(table_in)
    string_in = str.split(table_in, '.')
    seed = int(string_in[0])
    gtype = int(string_in[1])
...
pmid2graph('1234.1.csv')
pmid2graph('1234.2.csv')
pmid2graph('1234.3.csv')
